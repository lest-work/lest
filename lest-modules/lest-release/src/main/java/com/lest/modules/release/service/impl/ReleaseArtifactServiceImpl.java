...
public class ReleaseArtifactServiceImpl extends ServiceImpl<ReleaseArtifactMapper, ReleaseArtifact>
    ...
    public Page<ReleaseArtifactVO> pageQuery(ReleaseArtifactDTO dto) {
        ...
        Page<ReleaseArtifact> resultPage = baseMapper.selectArtifactPage(page, wrapper);

        Page<ReleaseArtifactVO> voPage = new Page<>(
    ...
    private ReleaseArtifactVO toVO(ReleaseArtifact artifact) {
        ReleaseArtifactVO vo = new ReleaseArtifactVO();
        vo.setId(artifact.getId());
        vo.setReleaseId(artifact.getReleaseId());
        ...
        vo.setFileHash(artifact.getFileHash());
        vo.setDownloadUrl(artifact.getDownloadUrl());
        vo.setDownloadCount(artifact.getDownloadCount());
        vo.setUploadedBy(artifact.getUploadedBy());
        vo.setUploadedByName(artifact.getUploadedByName());
        vo.setUploadedAt(artifact.getUploadedAt());
        vo.setReleaseName(artifact.getReleaseName());
        return vo;
    }
