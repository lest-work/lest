    ...
    exportConfig: {
      beforeExport: (params: any) => {
        ...
        workbook.xlsx.writeBuffer().then((data) => {
          download(data as unknown as Blob, `${fileName}.xlsx`);
          closeModal();
        });
        return false;
      }
 ...
 */
export function getExportWorkbook(params: any) {
  ...
  [...headerData, ...bodyData, ...footerData].forEach((item: any[], index: number) => {
    const sheetCols: any[] = [];
    const rowStyles: any[] = [];
    item.forEach((d: any, colIndex: number) => {
    ...
    const expandCell = item.find((d: any) => d.isExpandCell && d.colspan);
  ...
  sheetMerges.forEach((merges) => {
    sheet.mergeCells(merges as [number, number, number, number]);
  });
  bodyCols.forEach((col: any, colIndex: number) => {
