...
<script lang="ts" setup>
  ...
  const save = () => {
    formRef.value?.validate?.((valid) => {
      ...
      saveOrUpdate({
        ...form,
        // 前端 menuType: 0=目录, 1=菜单, 2=按钮 -> 后端: 1=目录, 2=菜单, 3=按钮
        menuType: form.menuType === 0 ? 1 : form.menuType === 1 ? 2 : 3,
  ...
  watch(visible, () => {
    if (visible.value) {
      if (props.data) {
        ...
        // 后端 menuType: 1=目录, 2=菜单, 3=按钮 -> 前端: 0=目录, 1=菜单, 2=按钮
        const menuType = props.data.menuType === 1 ? 0 : props.data.menuType === 2 ? 1 : 2;
        assignFields({
          ...props.data,
          menuType,
