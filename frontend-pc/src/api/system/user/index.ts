...
export async function getUser(id: number) {
  const res = await request.get<ApiResult<User>>('/auth/user/' + id);
  if (res.data.code === 200 && res.data.data) {
    return res.data.data;
  }
  return Promise.reject(new Error(res.data.message));
...
export async function addUser(data: User) {
  const res = await request.post<ApiResult<unknown>>('/auth/user', data);
  if (res.data.code === 200) {
...
export async function removeUser(id?: number) {
  const res = await request.delete<ApiResult<unknown>>('/auth/user/' + id);
  if (res.data.code === 200) {
    return res.data.message;
  }
  return Promise.reject(new Error(res.data.message));
...
export async function removeUsers(data: (number | undefined)[]) {
  const res = await request.delete<ApiResult<unknown>>('/auth/user/batch', {
    data
  });
  if (res.data.code === 200) {
    return res.data.message;
...
export async function updateUserStatus(userId?: number, status?: number) {
  const res = await request.put<ApiResult<unknown>>('/auth/user/status', {
    userId,
    status
  });
  if (res.data.code === 200) {
 ...
 * 重置用户密码
 */
export async function updateUserPassword(userId?: number, password = 'lest123456') {
  const res = await request.put<ApiResult<unknown>>('/auth/user/password', {
    userId,
    password
  });
  if (res.data.code === 200) {
...
export async function getCurrentUser() {
  const res = await request.get<ApiResult<unknown>>('/auth/current-user');
  if (res.data.code === 200 && res.data.data) {
    return res.data.data;
  }
  return Promise.reject(new Error(res.data.message));
