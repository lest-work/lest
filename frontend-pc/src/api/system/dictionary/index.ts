...
export async function listDictionaries(params?: DictionaryParam) {
  const res = await request.get<ApiResult<Dictionary[]>>('/auth/dictionary', {
    params
  });
  if (res.data.code === 200) {
    return res.data.data;
...
export async function updateDictionary(data: Dictionary) {
  const res = await request.put<ApiResult<unknown>>('/auth/dictionary', data);
  if (res.data.code === 200) {
    return res.data.message;
  }
  return Promise.reject(new Error(res.data.message));
