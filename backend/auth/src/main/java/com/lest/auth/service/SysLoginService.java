...
public class SysLoginService
{
    ...
    public LoginUser login(String username, String password)
    {
        ...
        // 为避免 LoginUser.sysUser 反序列化异常，单独调用原始用户接口获取 SysUser
        R<SysUser> rawResult = remoteUserService.getRawUser(username, SecurityConstants.INNER);
        if (rawResult == null || R.FAIL == rawResult.getCode() || rawResult.getData() == null)
        {
            String msg = rawResult != null && rawResult.getMsg() != null ? rawResult.getMsg() : "用户不存在";
            throw new ServiceException(msg);
        }
        SysUser user = rawResult.getData();
