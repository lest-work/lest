...
import com.lest.api.system.factory.RemoteUserFallbackFactory;
...
public interface RemoteUserService
{
    ...
    /**
     * 通过用户名查询原始用户对象（内部使用，仅返回 SysUser）。
     */
    @GetMapping("/user/raw/{username}")
    public R<SysUser> getRawUser(@PathVariable("username") String username, @RequestHeader(SecurityConstants.FROM_SOURCE) String source);

    /**
     * 通过邮箱查询用户信息（内部使用）。
     */
    @GetMapping("/user/email/{email}")
    public R<SysUser> getUserByEmail(@PathVariable("email") String email, @RequestHeader(SecurityConstants.FROM_SOURCE) String source);
