...
public class JwtTokenProvider {
    ...
    public void blacklistAccessToken(String token) {
        String jti = JwtUtils.getJti(token);
        if (jti == null) return;
        long remaining = JwtUtils.getAccessTokenExpirationSeconds();
        redisTemplate.opsForValue().set(ACCESS_BLACKLIST_PREFIX + jti, "1", remaining, TimeUnit.SECONDS);
        log.info("访问令牌已加入黑名单: jti={}", jti);
    }
