...
## Phase 7 — 批量操作（Batch Operations）

> **说明**：PRD 设计主要在 V2.0 中已覆盖（批量编辑/移动/删除/批量流转），V3.0 负责完整实现与验收，作为 Jira 100% 收口的一部分。

### 7.1 后端 API

||| 任务 ID | 说明 | 优先级 | 状态 |
||| :--- | :--- | :--- | :--- |
||| TSK-V3-BE-701 | BulkOperationController — 批量编辑 API（字段更新、标签、优先级、迭代等） | P1 | 🔴 待启动 |
||| TSK-V3-BE-702 | BulkOperationController — 批量移动（跨项目/跨 Issue Type） | P1 | 🔴 待启动 |
||| TSK-V3-BE-703 | BulkOperationController — 批量流转（按工作流校验可达状态） | P1 | 🔴 待启动 |
||| TSK-V3-BE-704 | BulkOperationController — 批量删除（进入回收站） | P1 | 🔴 待启动 |
||| TSK-V3-BE-705 | 批量操作审计与回滚策略（失败重试/部分成功提示） | P2 | 🔴 待启动 |

### 7.2 前端

||| 任务 ID | 说明 | 优先级 | 状态 |
||| :--- | :--- | :--- | :--- |
||| TSK-V3-FE-701 | Issue 列表多选 + 批量操作栏（预览影响数量） | P1 | 🔴 待启动 |
||| TSK-V3-FE-702 | 批量操作向导（分步：选择动作→配置→预览→执行） | P2 | 🔴 待启动 |

---

## Phase 8 — 投票/分享（Vote / Share）

> **说明**：作为 Jira 100% 对齐收口项，需补齐完整交互、权限、审计与通知。

### 8.1 后端 API

||| 任务 ID | 说明 | 优先级 | 状态 |
||| :--- | :--- | :--- | :--- |
||| TSK-V3-BE-801 | Vote API — 投票/取消投票/投票列表 | P2 | 🔴 待启动 |
||| TSK-V3-BE-802 | Share API — 生成分享链接/失效/访问鉴权（可配置过期时间） | P2 | 🔴 待启动 |

### 8.2 前端

||| 任务 ID | 说明 | 优先级 | 状态 |
||| :--- | :--- | :--- | :--- |
||| TSK-V3-FE-801 | 任务详情页 Vote 组件（计数 + 操作） | P2 | 🔴 待启动 |
||| TSK-V3-FE-802 | 分享弹窗（复制链接/权限提示/过期设置） | P2 | 🔴 待启动 |

---

## Phase 9 — 快捷键（Shortcuts）与自动化模板库

### 9.1 快捷键

||| 任务 ID | 说明 | 优先级 | 状态 |
||| :--- | :--- | :--- | :--- |
||| TSK-V3-FE-901 | 全局快捷键帮助面板（? 打开） | P2 | 🔴 待启动 |
||| TSK-V3-FE-902 | Board/Backlog 上下文快捷键（E=Edit、S=Sprint、←→ 调整顺序等） | P2 | 🔴 待启动 |

### 9.2 自动化规则模板库

||| 任务 ID | 说明 | 优先级 | 状态 |
||| :--- | :--- | :--- | :--- |
||| TSK-V3-BE-911 | AutomationTemplate API — 模板 CRUD + 一键创建规则 | P2 | 🔴 待启动 |
||| TSK-V3-BE-912 | 规则导入/导出（JSON） | P2 | 🔴 待启动 |
||| TSK-V3-FE-911 | 模板库页面（分类：分配/流转/通知/字段同步） | P2 | 🔴 待启动 |
