# UI 扩展点契约设计
> **定位**：本文档定义 LEST Platform 前端 UI 扩展点契约，使 Core 页面在不引入完整插件运行时的前提下，为 V4.0 插件生态预留可扩展位置。
>
> **Core-first 调整**：V2.0 不实现完整插件运行时、SDK、市场或热加载；V2.0 只沉淀 Extension Point 命名、页面插槽、上下文数据和渲染边界。完整前端插件运行时从 V4.0 开始规划。
>
> **前置阅读**：
...
1. **Core 先保持可用** — 任务详情页、项目详情页先由 Core 提供基础字段、评论、活动、看板等能力
2. **扩展点先命名和定边界** — V2.0 只确定哪些区域可扩展、扩展上下文包含什么、Core 如何保护布局
3. **插件实现后置** — 「工时」「CI/CD」「会议」等具体插件从 V4.0 插件运行时开始接入
...
## 3. 前端扩展声明（V4.0 SDK 参考）

> 本节用于定义 V4.0 插件 SDK 的目标形态。V2.0 仅需要在 Core 页面中预留对应扩展槽位和上下文类型，不需要实现动态加载、插件生命周期或市场能力。
...
> 本节描述 V4.0 的目标运行时，不作为 V2.0 开发任务。
...
### Phase 1：UI 扩展点契约（V2.0 Core）

1. 定义 Task / Project / Board / Dashboard 的扩展点 ID
2. 定义每类扩展点的上下文数据结构
3. 在 Core 页面预留稳定插槽和布局边界
4. 明确哪些扩展点只读、哪些扩展点允许触发命令
5. 把扩展点清单同步到 `core/EXTENSION-POINTS.md`

### Phase 2：插件运行时基础设施（V4.0）
...
本文档在 Core-first 路线下先定义 **前端扩展点契约**，为 V4.0 插件运行时提供输入。

| 文档 | 关注点 |
|------|--------|
| lest-plugin-sdk.md | 插件运行时、生命周期、API 路由、数据隔离、SDK 目标形态（V4.0） |
| **UI插件化架构设计.md** | **前端扩展点契约、页面插槽和上下文边界（V2.0 契约 / V4.0 运行时）** |

两者共同构成完整的 LEST 插件化体系，但完整实现不进入 V2.0 交付。
...
| 版本 | 日期 | 修改内容 |
|------|------|---------|
| V1.1 | 2026-06-03 | Core-first 调整：V2.0 只保留 UI 扩展点契约，完整插件运行时和 SDK 后移到 V4.0 |
